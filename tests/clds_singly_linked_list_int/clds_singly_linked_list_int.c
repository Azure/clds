...
{
    volatile LONG done;
    CLDS_SINGLY_LINKED_LIST_HANDLE sorted_list;
#ifdef _MSC_VER
    ...
    while (InterlockedAdd(&chaos_test_context->done, 0) != 1)
    {
        ...
        case 0:
            ...
            if (InterlockedCompareExchange(&chaos_test_context->items[item_index].item_state, TEST_LIST_ITEM_INSERTING, TEST_LIST_ITEM_NOT_USED) == TEST_LIST_ITEM_NOT_USED)
            {
                ...
                item_payload->value = item_index;

                ASSERT_ARE_EQUAL(int, 0, clds_singly_linked_list_insert(chaos_test_context->sorted_list, chaos_thread_data->clds_hazard_pointers_thread, chaos_test_context->items[item_index].item));
        ...
        case 1:
            ...
            {
                ASSERT_ARE_EQUAL(CLDS_SINGLY_LINKED_LIST_DELETE_RESULT, CLDS_SINGLY_LINKED_LIST_DELETE_OK, clds_singly_linked_list_delete(chaos_test_context->sorted_list, chaos_thread_data->clds_hazard_pointers_thread, chaos_test_context->items[item_index].item));
        ...
        case 2:
            ...
            //if (InterlockedCompareExchange(&chaos_test_context->items[item_index].item_state, TEST_LIST_ITEM_DELETING, TEST_LIST_ITEM_USED) == TEST_LIST_ITEM_USED)
            //{
            //    ASSERT_ARE_EQUAL(CLDS_SINGLY_LINKED_LIST_DELETE_RESULT, CLDS_SINGLY_LINKED_LIST_DELETE_OK, clds_singly_linked_list_delete_if(chaos_test_context->sorted_list, chaos_thread_data->clds_hazard_pointers_thread, chaos_test_context->items[item_index].item));
            //
            //    (void)InterlockedExchange(&chaos_test_context->items[item_index].item_state, TEST_LIST_ITEM_NOT_USED);
            //}
            break;
...
TEST_FUNCTION(clds_singly_linked_list_chaos_knight_test)
{
    ...
    chaos_test_context->sorted_list = clds_singly_linked_list_create(hazard_pointers);
    ...
    clds_singly_linked_list_destroy(chaos_test_context->sorted_list);

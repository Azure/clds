...
{
    volatile MPSC_LOCK_FREE_QUEUE_ITEM* dequeue_head;
    volatile MPSC_LOCK_FREE_QUEUE_ITEM* enqueue_head;
...
int mpsc_lock_free_queue_enqueue(MPSC_LOCK_FREE_QUEUE_HANDLE mpsc_lock_free_queue, MPSC_LOCK_FREE_QUEUE_ITEM* item)
{
    ...
    else
    {
        ...
        do
        {
            ...
            MPSC_LOCK_FREE_QUEUE_ITEM* current_head = InterlockedCompareExchangePointer((volatile PVOID*)&mpsc_lock_free_queue->enqueue_head, NULL, NULL);
            ...
            if (InterlockedCompareExchangePointer((volatile PVOID*)&mpsc_lock_free_queue->enqueue_head, (PVOID)item, (PVOID)current_head) == current_head)
            {
...
MPSC_LOCK_FREE_QUEUE_ITEM* mpsc_lock_free_queue_dequeue(MPSC_LOCK_FREE_QUEUE_HANDLE mpsc_lock_free_queue)
{
    ...
    else
    {
        ...
        result = InterlockedCompareExchangePointer((volatile PVOID*)&mpsc_lock_free_queue->dequeue_head, NULL, NULL);
        if (result != NULL)
        {
            // simply advance the dequeue head
            (void)InterlockedExchangePointer((volatile PVOID*)&mpsc_lock_free_queue->dequeue_head, (PVOID)result->next);
        }
        else
        {
            ...
            MPSC_LOCK_FREE_QUEUE_ITEM* temporary_list = InterlockedExchangePointer((volatile PVOID*)&mpsc_lock_free_queue->enqueue_head, NULL);
            ...
            if (temporary_list != NULL)
            {
                ...
                volatile MPSC_LOCK_FREE_QUEUE_ITEM* new_dequeue_head = NULL;
                volatile MPSC_LOCK_FREE_QUEUE_ITEM* current_item = temporary_list;

                while (current_item != NULL)
                {
                    volatile MPSC_LOCK_FREE_QUEUE_ITEM* next_item = current_item->next;
                ...
                {
                    (void)InterlockedExchangePointer((volatile PVOID*)&mpsc_lock_free_queue->dequeue_head, (PVOID)new_dequeue_head->next);
                    ...
                    result = (MPSC_LOCK_FREE_QUEUE_ITEM*)new_dequeue_head;
                }
            }
        }
    }
    ...
    else
    {
        ...
        if (result == NULL)
        {
            ...
            if (temporary_list != NULL)
            {
                ...
                (void)InterlockedExchangePointer((volatile PVOID*)&mpsc_lock_free_queue->dequeue_head, (PVOID)new_dequeue_head);
...
int mpsc_lock_free_queue_is_empty(MPSC_LOCK_FREE_QUEUE_HANDLE mpsc_lock_free_queue, bool* is_empty)
{
    ...
    else
    {
        ...
        if (InterlockedCompareExchangePointer((volatile PVOID*)&mpsc_lock_free_queue->dequeue_head, NULL, NULL) != NULL)
        {
        ...
        {
            if (InterlockedCompareExchangePointer((volatile PVOID*)&mpsc_lock_free_queue->enqueue_head, NULL, NULL) != NULL)
            {
                /* Codes_SRS_MPSC_LOCK_FREE_QUEUE_01_021: [ mpsc_lock_free_queue_is_empty shall set is_empty to false if at least one item is in the queue. ]*/
                *is_empty = false;
            }

...
int mpsc_lock_free_queue_enqueue(MPSC_LOCK_FREE_QUEUE_HANDLE mpsc_lock_free_queue, MPSC_LOCK_FREE_QUEUE_ITEM* item)
{
    ...
    else
    {
        ...
        do
        {
            ...
            MPSC_LOCK_FREE_QUEUE_ITEM* current_head = interlocked_compare_exchange_pointer(&mpsc_lock_free_queue->enqueue_head, NULL, NULL);
            ...
            if (interlocked_compare_exchange_pointer(&mpsc_lock_free_queue->enqueue_head, item, current_head) == current_head)
            {
...
MPSC_LOCK_FREE_QUEUE_ITEM* mpsc_lock_free_queue_dequeue(MPSC_LOCK_FREE_QUEUE_HANDLE mpsc_lock_free_queue)
{
    ...
    else
    {
        ...
        result = interlocked_compare_exchange_pointer(&mpsc_lock_free_queue->dequeue_head, NULL, NULL);
        if (result != NULL)
        {
            // simply advance the dequeue head
            (void)interlocked_exchange_pointer(&mpsc_lock_free_queue->dequeue_head, result->next);
        }
        else
        {
            ...
            MPSC_LOCK_FREE_QUEUE_ITEM* temporary_list = interlocked_exchange_pointer(&mpsc_lock_free_queue->enqueue_head, NULL);
            ...
            if (temporary_list != NULL)
            {
                ...
                {
                    (void)interlocked_exchange_pointer(&mpsc_lock_free_queue->dequeue_head, new_dequeue_head->next);
    ...
    else
    {
        ...
        if (result == NULL)
        {
            ...
            if (temporary_list != NULL)
            {
                ...
                (void)interlocked_exchange_pointer(&mpsc_lock_free_queue->dequeue_head, new_dequeue_head);
...
int mpsc_lock_free_queue_is_empty(MPSC_LOCK_FREE_QUEUE_HANDLE mpsc_lock_free_queue, bool* is_empty)
{
    ...
    else
    {
        ...
        if (interlocked_compare_exchange_pointer(&mpsc_lock_free_queue->dequeue_head, NULL, NULL) != NULL)
        {
        ...
        {
            if (interlocked_compare_exchange_pointer(&mpsc_lock_free_queue->enqueue_head, NULL, NULL) != NULL)
            {
                /* Codes_SRS_MPSC_LOCK_FREE_QUEUE_01_021: [ mpsc_lock_free_queue_is_empty shall set is_empty to false if at least one item is in the queue. ]*/
                *is_empty = false;
            }

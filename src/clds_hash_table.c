...
static BUCKET_ARRAY* get_first_bucket_array(CLDS_HASH_TABLE* clds_hash_table)
{
    ...
    BUCKET_ARRAY* first_bucket_array = interlocked_compare_exchange_pointer((volatile PVOID*)&clds_hash_table->first_hash_table, NULL, NULL);
    LONG bucket_count = interlocked_add(&first_bucket_array->bucket_count, 0);
    while (interlocked_add(&first_bucket_array->item_count, 0) >= bucket_count)
    {
        ...
        else
        {
            ...
            if (interlocked_compare_exchange_pointer((volatile PVOID*)&clds_hash_table->first_hash_table, new_bucket_array, first_bucket_array) == first_bucket_array)
            {
            ...
            else
            {
                ...
                first_bucket_array = interlocked_compare_exchange_pointer((volatile PVOID*)&clds_hash_table->first_hash_table, NULL, NULL);
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                (void)interlocked_exchange_pointer((volatile PVOID*)&clds_hash_table->first_hash_table->next_bucket, NULL);
                (void)interlocked_exchange(&clds_hash_table->first_hash_table->bucket_count, (LONG)initial_bucket_size);
                ...
                {
                    (void)interlocked_exchange_pointer(&clds_hash_table->first_hash_table->hash_table[i], NULL);
                }
...
void clds_hash_table_destroy(CLDS_HASH_TABLE_HANDLE clds_hash_table)
{
    ...
    else
    {
        ...
        BUCKET_ARRAY* bucket_array = interlocked_compare_exchange_pointer((volatile PVOID*)&clds_hash_table->first_hash_table, NULL, NULL);
        while (bucket_array != NULL)
        {
            BUCKET_ARRAY* next_bucket_array = interlocked_compare_exchange_pointer((volatile PVOID*)&bucket_array->next_bucket, NULL, NULL);
            ...
            for (i = 0; i < bucket_array->bucket_count; i++)
            {
                ...
                {
                    CLDS_SORTED_LIST_HANDLE linked_list = interlocked_compare_exchange_pointer(&bucket_array->hash_table[i], NULL, NULL);
                    if (linked_list != NULL)
                    {
...
CLDS_HASH_TABLE_INSERT_RESULT clds_hash_table_insert(CLDS_HASH_TABLE_HANDLE clds_hash_table, CLDS_HAZARD_POINTERS_THREAD_HANDLE clds_hazard_pointers_thread, void* key, CLDS_HASH_TABLE_ITEM* value, int64_t* sequence_number)
{
    ...
    else
    {
        ...
        BUCKET_ARRAY* next_bucket_array = interlocked_compare_exchange_pointer((volatile PVOID*)&find_bucket_array->next_bucket, NULL, NULL);
        ...
        {
            next_bucket_array = interlocked_compare_exchange_pointer((volatile PVOID*)&find_bucket_array->next_bucket, NULL, NULL);

            bucket_index = hash % interlocked_add(&find_bucket_array->bucket_count, 0);
            bucket_list = interlocked_compare_exchange_pointer(&find_bucket_array->hash_table[bucket_index], NULL, NULL);
        ...
        else
        {
            ...
            do
            {
                ...
                bucket_list = interlocked_compare_exchange_pointer(&current_bucket_array->hash_table[bucket_index], NULL, NULL);
                if (bucket_list != NULL)
                {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (interlocked_compare_exchange_pointer(&current_bucket_array->hash_table[bucket_index], bucket_list, NULL) != NULL)
                        {
...
CLDS_HASH_TABLE_DELETE_RESULT clds_hash_table_delete(CLDS_HASH_TABLE_HANDLE clds_hash_table, CLDS_HAZARD_POINTERS_THREAD_HANDLE clds_hazard_pointers_thread, void* key, int64_t* sequence_number)
{
    ...
    else
    {
        ...
        current_bucket_array = interlocked_compare_exchange_pointer((volatile PVOID*)&clds_hash_table->first_hash_table, NULL, NULL);
        while (current_bucket_array != NULL)
        {
            BUCKET_ARRAY* next_bucket_array = interlocked_compare_exchange_pointer((volatile PVOID*)&current_bucket_array->next_bucket, NULL, NULL);
...
CLDS_HASH_TABLE_SET_VALUE_RESULT clds_hash_table_set_value(CLDS_HASH_TABLE_HANDLE clds_hash_table, CLDS_HAZARD_POINTERS_THREAD_HANDLE clds_hazard_pointers_thread, void* key, CLDS_HASH_TABLE_ITEM* new_item, CLDS_HASH_TABLE_ITEM** old_item, int64_t* sequence_number)
{
    ...
    else
    {
        ...
        BUCKET_ARRAY* next_bucket_array = interlocked_compare_exchange_pointer((volatile PVOID*)&first_bucket_array->next_bucket, NULL, NULL);
        if (next_bucket_array != NULL)
        {
            // wait for all outstanding inserts in the lower levels to complete
            do
...
CLDS_HASH_TABLE_ITEM* clds_hash_table_find(CLDS_HASH_TABLE_HANDLE clds_hash_table, CLDS_HAZARD_POINTERS_THREAD_HANDLE clds_hazard_pointers_thread, void* key)
{
    ...
    else
    {
        ...
        BUCKET_ARRAY* current_bucket_array = interlocked_compare_exchange_pointer((volatile PVOID*)&clds_hash_table->first_hash_table, NULL, NULL);
        while (current_bucket_array != NULL)
        {
            BUCKET_ARRAY* next_bucket_array = interlocked_compare_exchange_pointer((volatile PVOID*)&current_bucket_array->next_bucket, NULL, NULL);
...
CLDS_HASH_TABLE_SNAPSHOT_RESULT clds_hash_table_snapshot(CLDS_HASH_TABLE_HANDLE clds_hash_table, CLDS_HAZARD_POINTERS_THREAD_HANDLE clds_hazard_pointers_thread, CLDS_HASH_TABLE_ITEM*** items, uint64_t* item_count)
{
    ...
    else
    {
        ...
        while (current_bucket_array != NULL)
        {
            ...
            if (interlocked_add(&current_bucket_array->item_count, 0) != 0)
            {
                LONG bucket_count = interlocked_add(&current_bucket_array->bucket_count, 0);
                LONG i;
        ...
        if (need_to_unlock_all)
        {
            ...
            BUCKET_ARRAY* bucket_array_to_clean = interlocked_compare_exchange_pointer((volatile PVOID*)&clds_hash_table->first_hash_table, NULL, NULL);
            while (bucket_array_to_clean != current_bucket_array)
            {
                BUCKET_ARRAY* next_bucket_array = interlocked_compare_exchange_pointer((volatile PVOID*)&bucket_array_to_clean->next_bucket, NULL, NULL);

                if (interlocked_add(&bucket_array_to_clean->item_count, 0) != 0)
                {
                    LONG bucket_count = interlocked_add(&bucket_array_to_clean->bucket_count, 0);

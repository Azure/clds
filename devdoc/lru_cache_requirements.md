...
`lru_cache` is module that implements least recently used cache using `clds_hash_table` and `doublylinkedlist` from `c_util`.

The module provides the following functionality:
- Inserting items in the cache.
- Get an item in the hash table by its key.
...
#define LRU_CACHE_PUT_RESULT_VALUES \
    LRU_CACHE_PUT_OK, \
    LRU_CACHE_PUT_ERROR, \
    LRU_CACHE_PUT_KEY_ALREADY_EXISTS_OK
...
MOCKABLE_FUNCTION(, LRU_CACHE_PUT_RESULT, lru_cache_put, LRU_CACHE_HANDLE, lru_handle, void*, key, CLDS_HASH_TABLE_ITEM*, value, int64_t, size);
...
**SRS_CLDS_HASH_TABLE_01_001: [** `lru_cache_create` shall create a new hash table object and on success it shall return a non-NULL handle to the newly created hash table. **]**
